%  m0804.m     (Matlab/Simulink R2007b)
%
%  Bild 8.4 b
...
%
%  Impulshöhe beta im Bereich   1. < beta < 4.
%
%   --->  s.u. im Programmfragment
%
% ########################################################
clear
...
a1=-exp(-T/T1); b1= 1+a1;  
ke=10;    
beta=4;                            % vgl.Beispiel 8.2
uek=zeros(1,ke+1); uek(1)=beta;     
uck(1)=0; 
for k=1:ke
    uck(k+1)= -a1*uck(k) + b1*uek(k);  % Ohne Strichpunkt:
                          %  Tabelle Bild 8.4, Spalte uc,k
end
k=0:ke;
% plot(k,uck,'x')
...
% der Ergebnisse so auf, wie sie Bild 8.4b zeigt:
...
fachs1=[0 0]; fachs2=[-.5 4.5];
plot(kachs1,kachs2,'k',fachs1,fachs2,'k')
grid on, hold on
set(gca,'FontSize',fonts,...
    'XLim',[-2 ke],'YLim',[-0.5 4.5],...    
    'XTick',[-2:10],'YTick',[-1:5]);
